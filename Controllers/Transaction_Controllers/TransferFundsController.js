...
const { CreditAlertMail, DebitAlertMail } = require("../../VIEW/mailDetails");
...
const handleTransferFunds = async (req, res) => {
  try {
    ...
    const receiverAC = await AuthModel.findOne({
    ...
    await SendEmail(senderAC.email, subject, message);
    ...
    await SendEmail(receiverAC.email, subject, message);
